...
import { buildSystemPrompt, buildUserPrompt } from '@/lib/work-support2/report/promptBuilder';
import { validateNodes } from '@/lib/work-support2/report/nodesValidator';
...
export async function POST(req: NextRequest) {
  ...
  await logAudit({ action: 'work-support2/report/generate', templateId, title });
  ...
  return NextResponse.json({ error: `문서 생성 실패 (${MAX_RETRIES}회 시도): ${lastError}` }, { status: 500 });
}
