...
import { buildSystemPrompt, buildUserPrompt } from '@/lib/work-support2/meeting/promptBuilder';
import { validateNodes } from '@/lib/work-support2/meeting/nodesValidator';
...
export async function POST(req: NextRequest) {
  ...
  const lenCheck = rejectIfTooLong(title, 200) || rejectIfTooLong(coreContent, 3000);
  if (lenCheck) return NextResponse.json({ error: lenCheck }, { status: 400 });

  await logAudit({ action: 'work-support2/meeting/generate', templateId, title });
  ...
  return NextResponse.json({ error: `회의자료 생성 실패 (${MAX_RETRIES}회 시도): ${lastError}` }, { status: 500 });
}
