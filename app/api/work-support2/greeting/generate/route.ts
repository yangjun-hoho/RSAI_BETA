...
import { buildSystemPrompt, buildUserPrompt } from '@/lib/work-support2/greeting/promptBuilder';
import { validateNodes } from '@/lib/work-support2/greeting/nodesValidator';
...
export async function POST(req: NextRequest) {
  ...
  const eventTitle = String(formData?.event_title || '');
  const coreContent = String(formData?.core_content || '');

  const piiCheck = rejectIfPii(eventTitle + ' ' + coreContent);
  if (piiCheck) return NextResponse.json({ error: piiCheck }, { status: 400 });

  const lenCheck = rejectIfTooLong(eventTitle, 200) || rejectIfTooLong(coreContent, 2000);
  if (lenCheck) return NextResponse.json({ error: lenCheck }, { status: 400 });

  await logAudit({ action: 'work-support2/greeting/generate', templateId, eventTitle });
  ...
  return NextResponse.json({ error: `인사말씀 생성 실패 (${MAX_RETRIES}회 시도): ${lastError}` }, { status: 500 });
}
