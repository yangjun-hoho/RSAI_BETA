...
import { buildSystemPrompt, buildUserPrompt } from '@/lib/work-support2/merit-citation/promptBuilder';
import { validateNodes } from '@/lib/work-support2/merit-citation/nodesValidator';
...
export async function POST(req: NextRequest) {
  ...
  const piiCheck = rejectIfPii(field + ' ' + coreContent);
  if (piiCheck) return NextResponse.json({ error: piiCheck }, { status: 400 });

  const lenCheck = rejectIfTooLong(field, 100) || rejectIfTooLong(coreContent, 2000);
  if (lenCheck) return NextResponse.json({ error: lenCheck }, { status: 400 });

  await logAudit({ action: 'work-support2/merit-citation/generate', templateId, field });
  ...
  return NextResponse.json({ error: `공적조서 생성 실패 (${MAX_RETRIES}회 시도): ${lastError}` }, { status: 500 });
}
