...
import PreviewPanel from '@/lib/work-support2/report/components/PreviewPanel';
...
export default function ReportPanel({ onBack }: Props) {
  ...
  useEffect(() => {
    handleTemplateSelect('report_basic');
  ...
  const handleTemplateSelect = async (id: string) => {
    ...
    const res = await fetch(`/api/work-support2/report/templates?id=${id}`);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/work-support2/report/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ templateId: selectedTemplate, formData: formValues }),
      });
      const data = await res.json();
  ...
  const sectionCount = activeNodes.filter(n => n.type === 'SECTION').length;
  const estimatedPages = Math.max(1, Math.ceil(activeNodes.length / 18));
  ...
  const previewContent = (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0, position: isFullscreen ? 'fixed' : 'relative', inset: isFullscreen ? 0 : 'auto', zIndex: isFullscreen ? 100 : 'auto', background: '#fff' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <PreviewPanel nodes={activeNodes} />
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', flex: 1, minWidth: 0, height: '100%', fontFamily: '"Noto Sans KR", sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ width: '360px', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', height: '48px', padding: '0 1rem', borderBottom: '1px solid #e5e7eb', background: '#f9fafb', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: '0.95rem', fontWeight: 700, color: '#111827' }}>보고서 생성</span>
        ...
        <div style={{ overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem', flex: 1 }}>
          {formSchema && (
            <>
              ...
              <button onClick={handleGenerate} disabled={isGenerating || !formValues.title} style={{ padding: '0.55rem', background: isGenerating ? '#9ca3af' : '#4f46e5', color: '#fff', border: 'none', borderRadius: '7px', fontWeight: 700, fontSize: '0.82rem', cursor: isGenerating ? 'not-allowed' : 'pointer' }}>
                {isGenerating ? '문서 생성 중...' : '문서 생성'}
              </button>
          ...
          <div style={{ overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1 }}>
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '0.875rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                {[{ label: '전체 항목', value: `${activeNodes.length}개` }, { label: '섹션 수', value: `${sectionCount}개` }, { label: '예상 페이지', value: `${estimatedPages}p` }].map(({ label, value }) => (
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '0.875rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {selectedTemplate && activeNodes.length > 0 && (
                  <DownloadButton templateId={selectedTemplate} nodes={activeNodes} fileName={String(formValues.title || 'report')} buttonStyle={{ width: '100%', padding: '0.5rem 0.75rem', fontSize: '0.82rem', fontWeight: 600 }} />
                )}
                <button onClick={handleCopyText} disabled={activeNodes.length === 0} style={{ width: '100%', padding: '0.5rem 0.75rem', border: '1px solid #d1d5db', borderRadius: '6px', background: '#fff', cursor: activeNodes.length === 0 ? 'not-allowed' : 'pointer', fontSize: '0.82rem', color: '#374151', fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', opacity: activeNodes.length === 0 ? 0.5 : 1 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '0.875rem' }}>
              ...
              <button onClick={handleGenerate} disabled={isGenerating || !selectedTemplate} style={{ width: '100%', padding: '0.5rem 0.75rem', border: '1px solid #c7d2fe', borderRadius: '6px', background: '#eef2ff', cursor: (isGenerating || !selectedTemplate) ? 'not-allowed' : 'pointer', fontSize: '0.82rem', color: '#4338ca', fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', opacity: !selectedTemplate ? 0.5 : 1 }}>
