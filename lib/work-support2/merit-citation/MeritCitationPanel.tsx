...
import MeritCitationPreviewPanel from '@/lib/work-support2/merit-citation/MeritCitationPreviewPanel';
import DownloadButton from '@/lib/work-support2/report/components/DownloadButton';

const TEMPLATE_ID = 'merit_citation_v2';
...
export default function MeritCitationPanel({ onBack }: Props) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/work-support2/merit-citation/generate', {
        method: 'POST',
      ...
      const fieldNode = data.nodes.find((n: DocumentNode) => n.type === 'FIELD');
      const title = fieldNode ? fieldNode.content.replace('■ 공적조서 분야 : ', '') : (formValues.field as string || '제목 없음');
  ...
  const previewContent = (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0, position: isFullscreen ? 'fixed' : 'relative', inset: isFullscreen ? 0 : 'auto', zIndex: isFullscreen ? 100 : 'auto', background: '#fff' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <MeritCitationPreviewPanel nodes={nodes} />
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', flex: 1, minWidth: 0, height: '100%', fontFamily: '"Noto Sans KR", sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ width: '360px', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', height: '48px', padding: '0 1rem', borderBottom: '1px solid #e5e7eb', background: '#f9fafb', flexShrink: 0 }}>
          <button onClick={onBack} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '28px', height: '28px', border: '1px solid #d1d5db', borderRadius: '6px', background: '#fff', cursor: 'pointer', color: '#6b7280', flexShrink: 0 }} title="뒤로가기">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 18l-6-6 6-6"/></svg>
          </button>
          <span style={{ fontSize: '0.95rem', fontWeight: 700, color: '#111827' }}>공적조서 생성</span>
        ...
        <div style={{ overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', flex: 1 }}>
          {formSchema && (
            <>
              ...
              <button onClick={handleGenerate} disabled={isGenerating || !formValues.field || !formValues.core_content} style={{ padding: '0.55rem', background: isGenerating ? '#9ca3af' : '#4f46e5', color: '#fff', border: 'none', borderRadius: '7px', fontWeight: 700, fontSize: '0.82rem', cursor: isGenerating ? 'not-allowed' : 'pointer' }}>
                {isGenerating ? '공적조서 생성 중...' : '공적조서 생성'}
              </button>
          ...
          <div style={{ overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1 }}>
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '0.875rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                {[
                  { label: '전체 항목', value: `${totalNodes}개` },
                  { label: '세부 성과', value: `${bulletCount}개` },
                  { label: '예상 페이지', value: totalNodes > 0 ? '1p' : '-' },
                ].map(({ label, value }) => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {nodes.length > 0 && (
                  <DownloadButton
                    ...
                    nodes={nodes}
                    fileName={String(formValues.field || 'merit-citation')}
                    apiPath="/api/work-support2/merit-citation/render"
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '0.875rem' }}>
              ...
              <button onClick={handleGenerate} disabled={isGenerating || !formValues.field || !formValues.core_content} style={{ width: '100%', padding: '0.5rem 0.75rem', border: '1px solid #c7d2fe', borderRadius: '6px', background: '#eef2ff', cursor: (isGenerating || !formValues.field || !formValues.core_content) ? 'not-allowed' : 'pointer', fontSize: '0.82rem', color: '#4338ca', fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', opacity: (!formValues.field || !formValues.core_content) ? 0.5 : 1 }}>
