...
import PressReleasePreviewPanel from '@/lib/work-support2/press-release/PressReleasePreviewPanel';
import DownloadButton from '@/lib/work-support2/report/components/DownloadButton';

const TEMPLATE_ID = 'press_release';
...
export default function PressReleasePanel({ onBack }: Props) {
  ...
  useEffect(() => {
    // 폼 스키마 로드 (report templates API 재사용)
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/work-support2/press-release/generate', {
        method: 'POST',
  ...
  const estimatedPages = paragraphCount > 0 ? Math.max(1, Math.ceil(paragraphCount / 4)) : 0;
  ...
  const previewContent = (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0, position: isFullscreen ? 'fixed' : 'relative', inset: isFullscreen ? 0 : 'auto', zIndex: isFullscreen ? 100 : 'auto', background: '#fff' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <PressReleasePreviewPanel nodes={nodes} />
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', flex: 1, minWidth: 0, height: '100%', fontFamily: '"Noto Sans KR", sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ width: '360px', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', height: '48px', padding: '0 1rem', borderBottom: '1px solid #e5e7eb', background: '#f9fafb', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: '0.95rem', fontWeight: 700, color: '#111827' }}>보도자료 생성</span>
          ...
          {formSchema && (
            <>
              ...
              <button onClick={handleGenerate} disabled={isGenerating || !formValues.title} style={{ padding: '0.55rem', background: isGenerating ? '#9ca3af' : '#4f46e5', color: '#fff', border: 'none', borderRadius: '7px', fontWeight: 700, fontSize: '0.82rem', cursor: isGenerating ? 'not-allowed' : 'pointer' }}>
                {isGenerating ? '보도자료 생성 중...' : '보도자료 생성'}
              </button>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <div style={{ overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {nodes.length > 0 && (
                  <DownloadButton
                    ...
                    nodes={nodes}
                    fileName={String(formValues.title || 'press-release')}
                    apiPath="/api/work-support2/press-release/render"
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '0.875rem' }}>
              ...
              <button onClick={handleGenerate} disabled={isGenerating || !formValues.title} style={{ width: '100%', padding: '0.5rem 0.75rem', border: '1px solid #c7d2fe', borderRadius: '6px', background: '#eef2ff', cursor: (isGenerating || !formValues.title) ? 'not-allowed' : 'pointer', fontSize: '0.82rem', color: '#4338ca', fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', opacity: !formValues.title ? 0.5 : 1 }}>
